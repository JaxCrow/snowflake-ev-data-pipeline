...
-- External Stage (Azure Blob with directory enabled for stream)
CREATE OR REPLACE STAGE AZURE_EV_STAGE
  STORAGE_INTEGRATION = AZURE_EV_STORAGE_INT
  URL = 'azure://evpipedev346059.blob.core.windows.net/landing/'
  DIRECTORY = (ENABLE = TRUE AUTO_REFRESH = TRUE NOTIFICATION_INTEGRATION = 'AZURE_EV_EVENTGRID_INT');
...
-- Directory stream (detects new files in Azure Blob stage)
CREATE OR REPLACE STREAM AZURE_FILES_STREAM
  ON STAGE AZURE_EV_STAGE;
...
def main(session):
    try:
        result = session.sql("""
          ...
          FROM @EV_PROJECT_DB.BRONZE.AZURE_EV_STAGE
...
CREATE OR REPLACE TASK TSK_INGEST_EV_DATA
  ...
  SCHEDULE = '1 MINUTE'
  WHEN SYSTEM$STREAM_HAS_DATA('EV_PROJECT_DB.BRONZE.AZURE_FILES_STREAM')
AS
